...
| S24      | Equipment, auras, attachments                                        | 7     | [#76](https://github.com/krakenhavoc/cmd_and_ctrl/issues/76)   | 2027-04-18 | **done**    |
...
| S30      | Damage prevention, cloning, face-down, deferred protection keywords  | 7     | [#95](https://github.com/krakenhavoc/cmd_and_ctrl/issues/95)   | 2027-09-05 | **done**    |
...
- [ ] Extend `runStepEntryHooksLocked` ([server/internal/game/game.go:327](../server/internal/game/game.go)):
  ...
  - `StepDraw`: auto-draw 1 card for `ActiveSeat`, **except** when `Turn.Seq == 1 && ActiveSeat == Game.StartingSeat` (CR 103.8a).
...
- **Infect / wither damage as counter placement** — handed to the combat keyword layer (S18 [#68](https://github.com/krakenhavoc/cmd_and_ctrl/issues/68)), which closed without it (ADR 0014 never covered infect); tracked with toxic in [#748](https://github.com/krakenhavoc/cmd_and_ctrl/issues/748) and **shipped** there (2026-09-24, [ADR 0056](decisions/0056-infect-wither-toxic.md) PR 2): infect, wither and toxic damage becomes -1/-1 counters and poison through the one damage tail. The poison-counter SBA ships here but the _creature-inflicts-poison_ mechanic does not
...
- **Combat-damage beats** — the engine runs first-strike and regular combat damage as two passes (CR 510.4), but every client sees both land in one frame. Presentation-only design (a `combat_step` tag on the log, beat cues on the combat arrows) in [ADR 0053](decisions/0053-combat-damage-beats.md); modelling the real second combat damage step is [#717](https://github.com/krakenhavoc/cmd_and_ctrl/issues/717). Polish work tracked on [#187](https://github.com/krakenhavoc/cmd_and_ctrl/issues/187).
...
~~Detailed plan TBD; lands just-in-time after S23.~~ The plan became [ADR 0036](decisions/0036-attachments.md) (the S32 spike, [#280](https://github.com/krakenhavoc/cmd_and_ctrl/issues/280)).

**Status: done** *(the row moved `partial` → `**done**` when [#726](https://github.com/krakenhavoc/cmd_and_ctrl/issues/726) landed; the checklist was reconciled 2026-09-16 at the #280 closeout)*. Every primitive and the card batch shipped, and #76 closed on 2026-09-14. The one thing holding the row down was the theme-deck smoke test, which #726 filed and then wrote; with the exit criterion met, [the status legend](#how-to-read-the-status-column) says `**done**` and the remaining tail does not hold the sprint open. That tail, all of it tracked:

- **Protection (CR 702.16)**, the seam #76's cut tables file Sword of Hearth and Home and the other protection cards under: [#662](https://github.com/krakenhavoc/cmd_and_ctrl/issues/662)
- **CR 704.5p** (an Aura or Equipment that stops being one): [#675](https://github.com/krakenhavoc/cmd_and_ctrl/issues/675)
- **Bruna, Light of Alabaster**: the three-zone multi-select prompt, recorded under S25 below
- **Everything else deferred**: the cut tables in the #76 issue body and the #555 PR body
...
- ~~**Invasion of New Phyrexia**~~ (`480ea052-87d7-4fea-ac97-b6121b78f863`) — **done in #626, and S27's card list is now 30 of 30.** The front is Hydroid Krasis's read-X-in-`OnResolve` pattern (X 2/2 white-and-blue Knights with vigilance, with the beat-early caveat that pattern carries). The back face, Teferi Akosa of Zhalfir, needed three different seams and got all three: its +1's "discard two cards unless you discard a creature card" is `DiscardPrompt` with the set-level `Validate` (#624) and a new `DiscardPrompt.Min`, because the floor of that clause is ONE and `Validate` is deliberately never asked about an empty pick — which is also the bug it fixes on Compulsive Research, the hook's first card, where `UpTo: true` let the target discard nothing; its −2 is a `Spec.Emblem` (#623, [ADR 0064](decisions/0064-emblems.md)) whose text includes a granted **ward**, built as `effects.WardGranted` — ward is a triggered ability with a cost (CR 702.21a), so the emblem carries the trigger rather than writing a keyword onto the Knight, and `WardAttached` and `Ward` are now the two narrow cases of it; its −3 is a `choose_cards` over your untapped creatures with a floor of ZERO plus a CR 603.12 reflexive trigger (#636) whose "mana value X or less" clause could not exist before, because X is not known until the taps are in.
...
- **Emblem ultimates** on Elspeth, Sun's Champion, Wrenn and Six and Teferi, Hero of Dominaria, plus the other omitted ultimates #572 lists, stay omitted rather than stubbed, per ADR 0032. Not on this checklist, but the same emblem seam as the New Phyrexia back face. Elspeth was the one planeswalker whose omission the catalog page did not publish — #418 left it `unreviewed` — and it now declares `caveats` like the others. **Superseded in S40** (#623, [ADR 0064](decisions/0064-emblems.md)): emblems are built, and Elspeth’s −7 and Teferi, Hero of Dominaria’s −8 both ship. Wrenn and Six’s −7 is still omitted, now on retrace and #652 rather than on emblems.
...
**Battles, follow-up 2026-09-14.** S27 shipped one battle of four (#415) and named the seam the other three were behind: *"a per-instance face on `ExilePlayPermission` and `faceOnResolve` honouring it"*. Both landed, plus the rule that a face-naming grant narrows the card's own offer rather than widening it — see the S32 addendum on [ADR 0034](decisions/0034-multi-face-cards.md). **All four battles** are now in — Innistrad, Karsus and Tarkir in #574, and Invasion of New Phyrexia in #626 — each with its back face registered under `"<oracle_id>#1"`, and the Siege reminder text finally does both halves: exile it, *then cast it transformed*. New Phyrexia was last because its back face, Teferi Akosa of Zhalfir, needed a conditional discard, an emblem with a granted ward and a reflexive shuffle-into-library: #624, #623 and #636 respectively, with the ward grant (`effects.WardGranted`) the one piece #626 itself built. The declared simplification for all four is timing, not faces — the free cast is a grant bounded to the turn of the defeat, cascade's trade for cascade's reason.
...
- [x] Warp (CR 702.185) — not on the original list; shipped under S29 as `Warp(cost)`: a discount from hand, the permanent exiled at the next end step, and an exile-play grant with a `NotBeforeSeq` floor for the later cast (#413). Weftstalker Ardent. [#324](https://github.com/krakenhavoc/cmd_and_ctrl/issues/324) stays open: Anticausal Vestige also puts a card from hand onto the battlefield ([#654](https://github.com/krakenhavoc/cmd_and_ctrl/issues/654))
...
- [x] Theme-deck smoke test (Avacyn + Reverberate + Clone + a fog plays 4 turns end-to-end) — `TestAvacynThemeDeckPlaysFourTurns` in `server/internal/cards/effects/theme_deck_avacyn_test.go` ([#678](https://github.com/krakenhavoc/cmd_and_ctrl/issues/678)). Four of the theme seat's turns, every card off `deck.ToGameCard` and every cast through Strict + AutoTap; the fog and the Reverberate go on an opponent's turn at instant speed, which is the only way either is ever really cast

**Status: done** *(the row moved `partial` → `**done**` when [#678](https://github.com/krakenhavoc/cmd_and_ctrl/issues/678) landed)*. Every primitive on the list except morph and protection has shipped, and S30's one exit criterion — the theme-deck test — is now written. By [the status legend](#how-to-read-the-status-column) that is `**done**`: the exit criterion is the bar, and the unshipped tail primitives are the same kind of tail the legend is explicit about not holding a sprint open for. The goal line ("after S30 there are no major missing primitives") is still overstated while morph and protection are out; that is a claim about the *goal*, not the exit criterion, and it is what the tail below is for. [#95](https://github.com/krakenhavoc/cmd_and_ctrl/issues/95) closed with the 2026-09-16 reconciliation (as #92 did at the S27 closeout), and the tail is tracked in its own issues:
...
- [x] Fixes a live bug in passing: `sorcery_speed` ships on the wire and the client never reads it. **Half of it fixed itself while this was in flight** — the S24 equip work (#379; ~~#380~~ *corrected 2026-09-16: #380 is S25's indestructible PR, not an attachment PR*) taught `contextMenu.logic.ts`'s `abilityBlocked` to honour the flag and landed `canActivateSorcerySpeedAbility`, which this branch had independently written under another name and now adopts. The half fixed here is `ManaAbilityMenu.svelte`'s own copy of `abilityBlocked`, which is the DEFAULT right-click popover (`adminOverrides` is off by default) and so the path most players were actually hitting
...
- [x] `PlayerView.is_bot` / `bot_tier` / `bot_deck`; protocol.ts and `docs/protocol.md` updated. (`docs/protocol.md` did not actually list the three fields until the S31 closeout on 2026-09-16. `docs/lobby.md` had them all along.) The three fields also ride the engine snapshot and the persisted lobby metadata, so a bot seat survives a deploy and the lobby's restore path relaunches its runner.
...
- **(6) is met.** Sub-PR 2 deleted the duplication, and the two-way agreement test keeps it deleted.

**Loop guard: descoped** (see sub-PR 3). **Hard tier with lookahead: descoped** in ADR 0033 §6 (see sub-PR 7).

### Follow-ups (S31 closeout, 2026-09-16)
...
- **Bot deckbuilding** — a bot never builds or tunes a deck. The picker offers the curated decks. `POST /games/{id}/seats/bot` also accepts a pasted decklist (`{tier, format, source}`), which the test harness uses. It goes through the same validation as a human upload and discloses uncatalogued cards in `unimplemented`.
