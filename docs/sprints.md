...
- [x] Saga chapter triggers + lore counter advance as turn-based action (CR 714) — `ChapterTrigger` / `ChapterTriggerTargeting` (`server/internal/cards/effects/saga.go`); the CR 714.2b entry and precombat-main lore counters and the CR 704.5s sacrifice in `server/internal/game/sagas.go` (#378)
...
**What is left, and what each piece is waiting on.** Neither is a card-file follow-up; each needs machinery whose shape was undecided at closeout. Decided 2026-09-16: emblems get an ADR first ([#623](https://github.com/krakenhavoc/cmd_and_ctrl/issues/623)); `ChooseCardsPrompt` gets a set-level `Validate` following `SearchLibrary` ([#624](https://github.com/krakenhavoc/cmd_and_ctrl/issues/624)); Heart of Kiran's alternative becomes a second ability with a real remove-a-loyalty-counter cost ([#625](https://github.com/krakenhavoc/cmd_and_ctrl/issues/625)); Invasion of New Phyrexia stays out until all three back-face abilities work ([#626](https://github.com/krakenhavoc/cmd_and_ctrl/issues/626)).

- **Invasion of New Phyrexia** (`480ea052-87d7-4fea-ac97-b6121b78f863`). The front is Hydroid Krasis's read-X-in-`OnResolve` pattern and is not the problem. The back face, Teferi Akosa of Zhalfir, is: its +1 is a conditional discard ("discard two cards unless you discard a creature card") that `ChooseCardsPrompt` cannot express because it has no set-level `Validate`; its −2 makes an **emblem**, and emblems have no shape (see [the seam registry](engine-seams.md)); its −3 is a reflexive trigger. Shipping it with two of three abilities omitted would be a 4-loyalty blank, which the #574 status comment on #92 rejected.
- **Heart of Kiran's alternative crew cost** — "you may remove a loyalty counter from a planeswalker you control rather than pay Heart of Kiran's crew cost". `AbilityCost` has no alternative-cost slot (the S22 `AlternativeCosts` are cast-time only) and no remove-a-counter-from-another-permanent component, and the client has no way to offer two ways to pay one ability. The card ships `caveats`, weaker than printed, and `heart_of_kiran.go` says so.
...
- [x] Ward (CR 702.21) — `TriggeredAbility` on `EventBecomesTarget` via S19; pay-or-counter prompt — `effects.Ward(WardMana(…))` (`server/internal/cards/effects/ward.go`) resolving into `QueuePayUnlessForEffect`. It was written in #421, which merged into a deleted base branch, and reached `develop` through the recovery PR #433. Mana wards only: life and sacrifice wards (Refraction Elemental, Sedgemoor Witch, Vein Ripper) are open PR #647, built on #552's Confirm and ChooseCards prompts with no new prompt kind
...
Also in flight from the same audit, as PRs rather than issues: Stunt Double (#641), Blood Money and The Battle of Bywater as one simultaneous sweep (#640), life and sacrifice ward (#647), the cost-option views' targeting gate (#639), and the face-down view leak (#646). Each engine seam above also has a row in [the seam registry](engine-seams.md).
