...
- **(4) is half met.** Layer A absorbs 91.3%, well past the 80% floor. Per-game model spend is still not measured: that needs a game against a hosted endpoint, and there is no key in CI, as [ADR 0033](decisions/0033-ai-bot-seat.md) §5's update says. #514's local OpenAI-compatible transport has no per-call bill to measure. **This is the only criterion not met, and it is what keeps the index row `partial`.** The row moves to `**done**` when a keyed game records its spend, or when the owner descopes the spend half.
