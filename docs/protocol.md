...
| `declare_attacker` | no | `{ "attacker": "<uuid>", "target": "<uuid>" }` | Marks a battlefield card as attacking the target player. Re-declaring the same attacker against a different target overwrites. Caller is not enforced (sandbox flexibility); rules-correct attacker validation lands with S13+. Added in S08. |
| `declare_blocker` | no | `{ "blocker": "<uuid>", "attacker": "<uuid>" }` | Marks a battlefield card as blocking the named attacker (also a battlefield card). Both must exist on the battlefield; attacker need not currently have `attacking_target` set. Added in S08. |
| `clear_combat` | no | — | Resets `attacking_target` and `blocking_target` on every battlefield card. Run at end of combat. Added in S08. |
