...
| `move_card` | no | `{ "src": <ZoneRef>, "dst": <ZoneRef>, "instance_id": "<uuid>", "as_commander"?: bool }` | General-purpose zone-to-zone move. Clears tapped state + counters if leaving the battlefield (CR 400.7). Caller must control the card. S13.1: when `as_commander` is true AND the card is a commander AND `dst` is graveyard / exile / hand / library, the destination is rewritten to the owner's command zone (CR 903.9 commander zone replacement, exercised as an explicit player choice rather than an automatic engine transform). |
...
- **EventView** (S14, omitempty): `{ kind, actor?, source?, target?, amount?, card_id?, old_zone?, new_zone?, error?, seq }` — one entry in the authoritative per-game event log. `kind` is one of `cast`, `resolve`, `fizzle`, `deal_damage`, `change_life`, `draw_card`, `discard_card`, `mill`, `zone_move`, `tap_card`, `untap_card`, `counter_placed`, `token_created`, `search_library`, `counter_spell`, `concede`, `etb`, `ltb`, `effect_error`. `seq` is monotonic within a game; `actor` / `source` / `target` / `card_id` are UUID strings keyed to the seated player / source card / target card. Rolling window on the wire (server-side cap) — pre-S14 replay snapshots decode as empty.
- **PendingChoiceView** (S14, omitempty): `{ id, kind, chooser, from_player, count, source, reason, options[] }` — one entry in the async effect-driven decision queue. `kind` is currently only `discard_from_hand`. `chooser` is the seat who picks (Thoughtseize — caster); `from_player` is the source zone owner (the discarder). `count` is how many cards must be picked. `source` is the card that produced the effect; `reason` is a human-readable tag (`"Thoughtseize"`) for client copy. `options[]` is an array of `CardView` entries — the candidate cards — materialised server-side from `from_player`'s live hand and redacted per-viewer through the same `known_by` projection used elsewhere (opponents snooping the wire still see backs). The chooser's client pops `ChoicePromptModal` and submits `resolve_choice` with the selected `card_ids`. Distinct from `discard_pending` (S13.4 cleanup-step over-max, chooser == discarder).
