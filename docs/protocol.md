...
- **Special actions** (CR 116.2 — foretell #658, suspend #659): DESIGNED, NOT BUILT. [ADR 0062](decisions/0062-abilities-and-special-actions-from-the-hand.md) Decision 4 records the shape so the two implementation issues add no new wire design: one `special_action` verb with `{ card_id, kind }`, `kind` one of `foretell` / `suspend` / later `turn_face_up`, no stack use, no targets, and per-kind timing (foretell is legal under split second, CR 702.61b; suspend is not, because it requires that you could begin to cast the card). Nothing ships it yet, and no frame carries it today.
