...
| `advance_step` | no | — | Advances the turn cursor by one step. Sandbox affordance — does NOT require the caller to hold priority (cf. `pass_priority` which does). Used by the "next step" / "done" buttons so the active player can drive combat progression without waiting for opponent priority passes. Auto-resolves combat damage on entry to a combat damage step (same hook as `pass_priority`): first-strike damage on entry to `first_strike_damage`, regular damage on entry to `combat_damage`. A step this turn does not have is walked through, not entered — so an advance out of `declare_blockers` lands on `combat_damage` in a combat with no first or double strike in it, and on `first_strike_damage` in one that has (#717). Added in S08. |
...
- **`step` values**, in turn order: `untap`, `upkeep`, `draw`, `precombat_main`, `begin_combat`, `declare_attackers`, `declare_blockers`, `first_strike_damage`, `combat_damage`, `end_combat`, `postcombat_main`, `end`, `cleanup`. `phase` is derived: `beginning`, `precombat_main`, `combat`, `postcombat_main`, `ending`.

  **Not every turn has every step.** `first_strike_damage` (#717, CR 506.1 / 510.4) is the first of the two combat damage steps, and a combat only has it when at least one attacking or blocking creature has first strike or double strike as it would begin. When it does not, the cursor never enters it: no `TurnView.step` ever names it, and no `step` log entry announces it. `combat_damage` is unchanged — it is every combat's regular (second) damage step, and it always happens. Both steps grant priority like any other, so the window between them is an ordinary priority window; clients must not special-case it. Added in S37.
- **StackItemView** (S13.1): `{ id, kind, controller, owner, source_card_id, label?, targets?, modes?, x_value?, distribution?, hold_priority?, split_second?, doubled_by?, doubled_by_name? }` — announce-time metadata for one item on the stack. `kind` is `"spell"` (card lives in `stack` zone), `"activated"` or `"triggered"` (no card; source stays in its origin zone). `targets` is a list of `TargetRefView` slots: `{ kind: "player" | "card" | "self" | "none", id?: "<uuid>" }`. Resolution re-checks targets per CR 608.2b — if every targeted slot is illegal, the spell is "countered by game rules" and routes to its owner's graveyard. `PlayerView.commander_casts` (S13.1, omitempty) is the per-commander cast tax counter map keyed by commander UUID string. `CardView.damage_marked` (omitempty) drives the lethal-damage SBA. **#764:** a `TargetRefView` may additionally carry `slot` (the target CLAUSE it answered) and `mode` (the index into `modes` — the mode OCCURRENCE — whose clause list that is), both omitted at zero; and the item carries `mode_labels?: [string]`, the oracle bullet of each chosen mode in announce order and with repeats. The labels travel with the item because the caster's hand card is gone by the time the spell is on the stack, and `modes: [0, 2]` is not something a table can read.
- **Target clauses and modes** (#764, [ADR 0065](decisions/0065-modal-and-multi-target-clauses.md)): a target statement is an ordered list of CLAUSES, each with its own predicate and count. `CardView.legal_targets` is clause 0 and is unchanged; `CardView.clauses?: [LegalTargetsView]` carries the whole list and is present only when there is more than one, so every pre-#764 client path keeps working. A `LegalTargetsView` gains `label?` (the clause's printed wording, for the picker banner) and `distinct?` (this clause's picks must differ from every earlier clause's — "a second target permanent you control"). `ModeOptionView` gains `clauses?` for the same reason, `ModeSpecView` gains `repeatable?` (CR 700.2d), and `ActivatedAbilityView` gains both `clauses?` and `modes?` — a modal activated ability announces its bullets with its targets (CR 602.2b). The client walks the clauses one prompt at a time and stamps each pick with the `slot` / `mode` it answered.
