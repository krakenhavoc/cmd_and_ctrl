...
| this repo | `CMDCTRL_R2_REPOSITORY`, `CMDCTRL_DEV_R2_REPOSITORY` | variables; the restic repository URL for each host's bucket, `s3:https://<account_id>.r2.cloudflarestorage.com/<bucket>`. See [Backups](#backups) |
| this repo | `CMDCTRL_R2_ACCESS_KEY_ID`, `CMDCTRL_R2_SECRET_ACCESS_KEY`, `CMDCTRL_DEV_R2_ACCESS_KEY_ID`, `CMDCTRL_DEV_R2_SECRET_ACCESS_KEY` | secrets; an R2 API token per host, scoped to that host's bucket. Written to `backup.env` |
| this repo | `CMDCTRL_RESTIC_PASSWORD`, `CMDCTRL_DEV_RESTIC_PASSWORD` | secrets; the restic repository keys. **Never change them once the repository exists**, and keep a copy in the password manager: it is the only way to read a backup if GitHub's copy is lost, since secrets are write-only |
...
These are repo-level Actions values, like every other per-host value in
[Secrets](#secrets). Production reads the `CMDCTRL_` names and the
preview reads the `CMDCTRL_DEV_` names:

| Kind | Production | Preview | Value |
|---|---|---|---|
| variable | `CMDCTRL_R2_REPOSITORY` | `CMDCTRL_DEV_R2_REPOSITORY` | `s3:https://<account_id>.r2.cloudflarestorage.com/<bucket>` |
| secret | `CMDCTRL_R2_ACCESS_KEY_ID` | `CMDCTRL_DEV_R2_ACCESS_KEY_ID` | the Access Key ID of an R2 API token scoped to that bucket |
| secret | `CMDCTRL_R2_SECRET_ACCESS_KEY` | `CMDCTRL_DEV_R2_SECRET_ACCESS_KEY` | that token's Secret Access Key |
| secret | `CMDCTRL_RESTIC_PASSWORD` | `CMDCTRL_DEV_RESTIC_PASSWORD` | the restic repository key |

If any of the four is unset for the host being deployed, CD emits a
`::warning::` naming each missing one, then stops and disables the
timer. The deploy stays green. With all four set, CD installs `restic`
...
4. **Set the values.** The account ID is on the R2 overview page.
   ...
   ```sh
   gh variable set CMDCTRL_R2_REPOSITORY     --body 's3:https://<account_id>.r2.cloudflarestorage.com/<prod-bucket>'
   gh variable set CMDCTRL_DEV_R2_REPOSITORY --body 's3:https://<account_id>.r2.cloudflarestorage.com/<dev-bucket>'
   gh secret set CMDCTRL_R2_ACCESS_KEY_ID          # each one prompts for the value
   gh secret set CMDCTRL_R2_SECRET_ACCESS_KEY
   gh secret set CMDCTRL_RESTIC_PASSWORD
   gh secret set CMDCTRL_DEV_R2_ACCESS_KEY_ID
   gh secret set CMDCTRL_DEV_R2_SECRET_ACCESS_KEY
   gh secret set CMDCTRL_DEV_RESTIC_PASSWORD
   ```
