...
**Two exceptions, both owned by this repo and installed by the CD job
on every deploy.** Caddy's routing table is
...
by `scripts/set-server-env.sh` from the "Sync server env" steps. Change
either one here, not in HomeLab.
