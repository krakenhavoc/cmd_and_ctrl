...
## Amendment (2026-09-23, #1313): Decision 8's second bullet is closed — untap holds with a CR 611.2b duration

[#1313](https://github.com/krakenhavoc/cmd_and_ctrl/issues/1313) builds the
"duration-bound restrictions" bullet of Decision 8, the one ADR 0070 Decision 6
designed and left unbuilt. The printed shape is:
...
> — Ty Lee, Chi Blocker (and Dungeon Geists, Tidebinder Mage, Icefall Regent,
> Merchant Raiders, Time of Ice, Shipbreaker Kraken)
>
> "{1}, {T}: Tap target artifact. It doesn't untap during its controller's untap
> step **for as long as this creature remains tapped**." — Rust Tick (and Amber
> Prison)

**Rules.** CR 611.2b: *"Some continuous effects generated by the resolution of a
spell or ability have durations worded 'for as long as . . . .' If the 'for as
long as' duration never starts, the effect does nothing. It doesn't start and
immediately stop again, and it doesn't last forever."* CR 502.3 is the untap
step these effects narrow. The Dungeon Geists rulings (2019-07-12, checked
against Scryfall on 2026-09-23) settle the edge cases:

- "If Dungeon Geists leaves the battlefield before its triggered ability has
  resolved, the target creature will be tapped, but it will be able to untap as
  normal."
- "If another player gains control of Dungeon Geists, its effect expires. It
  won't keep the creature from untapping anymore, even if you later regain
  control of Dungeon Geists."
- An already-tapped target "just remains tapped and won't untap during its
  controller's untap step."

"Its controller's" is read at each untap step, as Decision 2 reads it for the
one-shot marker. So a held creature that changes controller stays held through
its new controller's untap steps.

### Decision A — the linked effect is an `UntapSkip` that carries a `Duration`

Decision 8 said these need "a per-source linked-object field". They do not need
one on the **source**. The effect is about the target, the target already has a
per-object list of reasons it will not untap (`Card.NextUntapSkips`, Decision 2),
and that list is already cloned, snapshotted, projected and cleared on a zone
change. So the entry goes there, with one more field:

```go
// untap.go
type UntapSkip struct {
    Player uuid.UUID // as before: nil = "its controller's"
    While  *Duration // NEW: nil = one-shot "next untap step" marker;
                     // non-nil = a hold for as long as the duration lasts
}
```

ADR 0070 Decision 6 proposed `Source` and `SourceEnteredAt` fields here. This
takes the same information in the `Duration` model's own type (ADR 0063,
`game/duration.go`), for three reasons:

1. **One expiry function.** A hold is live exactly when
   `durationExpiredLocked(*While, false)` says so. The engine already has one
   function that decides when every continuous effect is over. Two bare fields
   would have made the untap reader a second copy of it.
2. **"You control" comes for free.** `WhileYouControlSource` already carries
   the player who must keep controlling the source, and the CR 400.7 entry
   stamp. Bare `Source` / `SourceEnteredAt` fields could not say "you control".
3. **"Remains tapped" is one more condition.** It is the new
   `WhileSourceRemainsTapped` `DurationCondition`, which is the name ADR 0070
   Decision 6 used. It is appended to the enum, so values already in snapshots
   keep their meaning. A `ScopedStatic` could use it too (Willow Satyr's control
   effect). The layer listener already bumps the layer version on
   `EventTapCard` and `EventUntapCard`, so both sweeps see the change.

The pointer is **immutable after registration**, as `ScopedStatic.Duration`
is. `cloneCard` copies the slice but shares each `*Duration` with the undo
snapshot, and this is safe because nothing writes through it.

### Decision B — read at the step, not consumed by it

- **Reader.** `untapSkippedForLocked(c, player)` replaces the
  `Card.hasNextUntapSkipFor` method. It is a method on `*Game` because a hold
  has to look at the board. A hold applies at the untap step of the player it
  is keyed to, which is "its controller's" (`Player == uuid.Nil`) for every
  printed card, and only while its duration holds. So a Seedborn Muse untap
  during another player's untap step still untaps a held permanent, as it does
  for a Decision 1 restriction. The printed text names only the controller's
  untap step.
- **Not consumed.** `consumeUntapSkipsLocked` leaves a live hold alone and
  drops an expired one, while it uses up one-shot markers as before.
- **Swept at the end of every layer pass.** `recomputeLayersLocked` calls
  `sweepUntapHoldsLocked` after `materialiseControlLocked`, and it drops
  every expired hold. The sweep does not run with the scoped statics at the
  top of the pass. At that point the board has not yet taken this pass's
  control changes, so a Threaten that ends before the next pass would never be
  seen. After materialisation, the pass that moves control of Ty Lee is also
  the pass that ends the hold. This keeps "it doesn't last forever" true in
  the case the second ruling describes: lose control of Ty Lee, get it back,
  and the creature still untaps. The reader re-checks the duration anyway, so
  the sweep only makes the end permanent. It is not what makes the effect
  correct at any single moment. Holds are not layer inputs, so the sweep does
  not bump the layer version.
- **Never starts.** The card-facing helpers build the duration through
  `ForAsLongAsYouControlDuration` / `ForAsLongAsSourceTappedDuration`. Those
  return `false` when the source is already gone, controlled by someone else,
  or untapped. When that happens the target is tapped and no hold is recorded.
  That is CR 611.2b and the Dungeon Geists ruling.
- **Zone change, copies, phasing.** These work as they did for Decision 2,
  because the hold is on the same slice. The target leaving the battlefield
  clears it (CR 400.7). A copy does not inherit it (CR 707.2). A
  phased-out **source** is not on the battlefield slice, so the duration reads
  false and the next sweep drops the hold, which matches CR 702.26f's "for as
  long as" corner.

### Decision C — the wire and the bots need no new field

A live hold means the permanent does not untap during its controller's untap
step **right now**. That is what `CardView.no_untap.static` already says
(Decision 4). So `stampNoUntap` sets `static` for a live hold, and a face-down
permanent keeps it: the hold comes from another object, not from the face-down
permanent's own (absent) abilities. `next` lists only one-shot markers.
Everything that reads `static` works without changes: the "won't untap" badge,
the hover-footer line ("doesn't untap during its controller's untap step"), the
heuristic's frozen-source weights, and the auto-tapper's `Frozen` ordering (the
last goes through `untapSkippedForLocked`). No new prompt, legal move or action
is added, so the enumerator has nothing new to list.

The footer does not say *which* source holds the permanent. Naming the source
would need a new wire field and a client string. That is a UX follow-up, not a
rules gap, and it is not built here.

### Decision D — the effect API
...
func (g *Game) UntapHeldLocked(c *Card) bool // the wire's reading

// effects
type DoesntUntapWhile struct{ Targets []uuid.UUID; Duration game.Duration }
...
A hold identical to one the permanent already has is not added again. Two
different sources (two Dungeon Geists on one creature) each record their own
hold, and each ends on its own.

### Proof cards

Ty Lee, Chi Blocker (the deck card, #1306). Its prowess is
[#706](https://github.com/krakenhavoc/cmd_and_ctrl/issues/706) and stays a
declared caveat. The other proof cards are Dungeon Geists, Tidebinder Mage, and
Rust Tick and Amber Prison, which lose their ADR 0070 Decision 6 caveats.

### Still out of scope

- Icefall Regent: its lock is writable now, but "spells your opponents cast
  that target this creature cost {2} more" has no cost-modifier shape. Merchant
  Raiders, Time of Ice (a Saga) and Shipbreaker Kraken (monstrosity) have not
  been checked for their other needs.
- Spider-Woman, Secret Agent: "can't become untapped" is not an untap-step
  restriction. It stops every untap (CR 701.26b), so it belongs in the untap
  primitive, next to stun.
- Willow Satyr, Rubinia Soulsinger and Hivis of the Scale ("gain control …
  for as long as you control this creature and this creature remains tapped")
  need a condition that is both of these at once. The enum can express that
  later, but no card here uses it.
