# ADR 0077 — Opponent boards are read-outs, not small cards: the summary panel and what expands it
...
**Numbering:** this ADR was written as 0076 and renumbered to **0077** on
2026-09-19, because [ADR 0076](0076-tutorial.md) merged to `develop` the same
day and claimed that number first. The original enumeration (0001–0075 on
`develop`, with 0005, 0024, 0029 and 0030 permanently unused per AGENTS.md §4,
plus `docs/decisions/` on `feat/1032-engine-table-settings`,
`feat/1038-rotate-invite` and `docs/s33-persistence-plan`, plus every branch
name matching `docs/adr-NNNN-*`) was **weaker than the AGENTS.md recipe** — the
repo has ~300 remote heads and they were not all tree-listed, because this
session reaches the repo only through the GitHub MCP and cannot run
`git log --all`. A number claimed on an unlisted branch and merged the same
afternoon is exactly the case that weakness could not rule out, and it is what
happened. `TestADRNumbersAreUniqueAndMatchTheirHeading` is what caught it, in
CI, which is the backstop working as designed; 0077 carries the same risk and
the same backstop.
