...
// TestTurnScopedStaticAppliesImmediately is the headline: a
// floating +3/+3 registered with no permanent behind it is visible
// on the very next recompute.
func TestTurnScopedStaticAppliesImmediately(t *testing.T) {
	g := newActiveGame(t)
...
// TestTurnScopedStaticExpiresAtCleanup walks the cursor through the
// cleanup step and checks the grant is gone — both from the
// registry and from the card's effective characteristic.
func TestTurnScopedStaticExpiresAtCleanup(t *testing.T) {
	g := newActiveGame(t)
	bear := pushScopedTestCreature(g, g.Seats[0].ID, 2, 2)
...
// TestTurnScopedStaticFromEndStepExpiresSameTurn is the CR 514.2
...
func TestTurnScopedStaticFromEndStepExpiresSameTurn(t *testing.T) {
	g := newActiveGame(t)
	bear := pushScopedTestCreature(g, g.Seats[0].ID, 2, 2)
	...
	g.WithWriteLock(func() {
		...
		}, uuid.New(), "end-step grant")
	})
	if p, _ := scopedEffectivePT(t, g, bear); p != 5 {
		t.Fatalf("setup: power = %d, want 5", p)
	}
	if got := g.TurnScopedStatics[0].ExpiresAfterTurn; got != turnAtGrant {
		t.Fatalf("ExpiresAfterTurn = %d, want the turn it was created on (%d)", got, turnAtGrant)
	}
...
// TestTurnScopedStaticComposesWithAnthemInLayerOrder is the CR
...
func TestTurnScopedStaticComposesWithAnthemInLayerOrder(t *testing.T) {
	g := newActiveGame(t)
...
// TestTurnScopedStaticsSortByTimestampWithinALayer is the CR 613.7
// pin proper: two effects in the SAME sub-layer, where order is
// observable because the later one overwrites the earlier.
func TestTurnScopedStaticsSortByTimestampWithinALayer(t *testing.T) {
	g := newActiveGame(t)
	bear := pushScopedTestCreature(g, g.Seats[0].ID, 2, 2)
...
// TestTurnScopedStaticSurvivesItsSourceLeaving is the reason the
// registry exists at all: the effect must outlive the card that
// made it. A battlefield static would vanish with its source.
func TestTurnScopedStaticSurvivesItsSourceLeaving(t *testing.T) {
	g := newActiveGame(t)
	owner := g.Seats[0].ID
	...
	g.WithWriteLock(func() {
		...
		}, sourcePermanent, "grant from a permanent that is about to die")
	})
	...
	if got := g.TurnScopedStatics[0].Source.InstanceID; got != sourcePermanent {
		t.Errorf("stored source LKI = %s, want %s", got, sourcePermanent)
	}
}

// TestCloneCopiesTurnScopedStatics — an undo snapshot taken while a
...
func TestCloneCopiesTurnScopedStatics(t *testing.T) {
	g := newActiveGame(t)
	g.WithWriteLock(func() {
		g.RegisterTurnScopedStaticForEffect(StaticAbility{
			Layer:     Layer6Ability,
	...
	snap := g.Clone()
	if len(snap.TurnScopedStatics) != 1 {
		t.Fatalf("clone dropped the live scoped static")
	}

	g.WithWriteLock(func() { g.ClearExpiredTurnScopedStaticsLocked() })
	if len(g.TurnScopedStatics) != 0 {
		t.Fatalf("sweep left %d entries on the original", len(g.TurnScopedStatics))
	}
	if len(snap.TurnScopedStatics) != 1 {
		t.Error("sweeping the original reached the clone (aliased slice)")
	}
	if snap.TurnScopedStatics[0].Label != "grant" {
		t.Errorf("clone's entry = %q, want %q", snap.TurnScopedStatics[0].Label, "grant")
	}
}

// TestRestoreFromRollsBackTurnScopedStatics is the undo direction:
// a grant registered after the snapshot must be gone once the
// snapshot is restored, and the restored board must recompute
// without it.
func TestRestoreFromRollsBackTurnScopedStatics(t *testing.T) {
	g := newActiveGame(t)
	bear := pushScopedTestCreature(g, g.Seats[0].ID, 2, 2)
	snap := g.Clone()
...
// TestClearExpiredTurnScopedStaticsIsIdempotent — the cleanup hook
...
func TestClearExpiredTurnScopedStaticsIsIdempotent(t *testing.T) {
	g := newActiveGame(t)
	g.WithWriteLock(func() {
		g.RegisterTurnScopedStaticForEffect(StaticAbility{
			Layer:     Layer6Ability,
