...
// for, and why it sets NarrowToCommanderIdentity (every "any color"
// source without that clause offers all five colours instead).
//
// The S14 note claiming the pipeline was still pending outlived its
// fix; corrected in the #338 stale-simplification sweep.
func init() {
	// Arcane Signet's produced string uses the full pipe set;
	// NarrowToCommanderIdentity makes the engine narrow the options to the controller's commander's
