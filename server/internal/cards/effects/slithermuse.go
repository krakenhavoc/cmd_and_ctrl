...
//   - "Choose an opponent" is auto-picked as the opponent holding
//     the most cards, which is always the choice that draws the
//     most. A player-choice prompt would need a trigger-side
//     player picker.
//   - Evoke ({3}{U}, sacrificed on entry) is an alternative cost
//     the engine has no shape for, so the card is castable only for
//     its printed {2}{U}{U}.
func init() {
	Register(Spec{
