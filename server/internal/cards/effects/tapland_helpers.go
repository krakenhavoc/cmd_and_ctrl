...
// dualManaAbility is "{T}: Add {A} or {B}" as one pipe ability, the
// shape the Temple cycle established. Two separate fixed-colour
// abilities would also work but would clutter the activation menu
// with a pair the player always reads as one choice.
