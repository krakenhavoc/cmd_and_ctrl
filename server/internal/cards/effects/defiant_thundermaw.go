...
// the attacker's id is captured in Build off the event rather than
// defaulting to item.SourceCardID the way most damage effects do.
//
// No simplification.
func init() {
	Register(Spec{
		...
		Triggered: []game.TriggeredAbility{{
			...
			Build: func(ev game.Event, source *game.Card, _ game.Characteristic, _ *game.Game) *game.StackItem {
				// Capture the UUID, never the *Card: the trigger
				// resolves after the battlefield slice may have been
				// reallocated under it.
				attacker := ev.CardID
				return game.NewTriggeredItem(source, "Defiant Thundermaw — the attacking Dragon deals 2 damage to any target",
					func(g *game.Game, item *game.StackItem) error {
						return thundermawDamage(g, item, attacker)
					})
			},
		}},
	})
...
//
// Caller holds g.mu.
func thundermawDamage(g *game.Game, item *game.StackItem, attacker uuid.UUID) error {
