...
//     replacement pinned to the returned OBJECT that redirects any
//     battlefield exit of it to exile — so sacrificing the whipped
//     creature in response to the end-step trigger does NOT put it
//     back in the graveyard for a second whip. Without this clause the
//     card would be stronger than printed; with it, the loop is closed
//     as printed.
//
// Both sandbox gaps this card used to declare are closed:
//
...
//   - CLOSED (#1591) — the redirect used to live in a turn-scoped
//     registry swept at cleanup, so a creature whose end-step exile
//     was countered (Stifle, #1211/#1223) survived the turn without
//     it and could die into the graveyard for a second whip. It is a
//     ScopedEffect record now (ADR 0041 phase 3 tier 3b), indefinite
//     and pinned to the returned object: it lasts exactly as long as
//     that object is on the battlefield, which is what the printed
//     clause says. See TestWhipRedirectOutlivesACounteredExile.
func init() {
	Register(Spec{
