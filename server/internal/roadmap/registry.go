...
var items = []Item{
	...
	{
		Slug: "infect-wither-toxic", Name: "Infect, wither and toxic", Kind: KindSeam, Status: StatusMissing,
		Summary:  "Damage that becomes -1/-1 counters on creatures, and poison counters on players.",
		Missing:  "Damage from a creature with infect, wither or toxic is dealt as ordinary damage; the -1/-1 counters and poison counters it should give aren't implemented.",
		Rules:    []string{"702.80", "702.90", "702.164"},
		Issue:    748,
		ADR:      "0056-infect-wither-toxic.md",
		Unblocks: 25,
		Waiting: []string{
			"Plague Myr", "Tainted Strike", "Blighted Agent", "Ichor Rats", "Karumonix, the Rat King",
			"Bloated Contaminator", "Grafted Exoskeleton", "Triumph of the Hordes",
		},
		Phrases:     []string{"infect", "wither", "toxic"},
		EngineNotes: "primitive: damage always marks damage or costs life. **[ADR 0056](decisions/0056-infect-wither-toxic.md) PR 1 has landed**: counters on players go through the CR 614 window (`ReplacementEvent.CounterPlayer`), every counter placement names who put it (`CounterPlacer` → the event's `Actor`), `player_counter_placed` bumps the layer version, and the `RepEventCounter` CR 616 resume sweeps SBAs and tolerates a vanished target. What is left is the damage tail itself (PR 2): the keyword tokens and the two branches below. Nothing turns a damage result into -1/-1 counters (wither, infect; CR 120.3d) or poison counters (infect, CR 120.3b), and nothing adds toxic's poison on combat damage (CR 702.164c). Poison as a player counter and its SBA already exist. Re-checked 2026-09-24: `game/infect_wither_toxic.go` holds the grammar and the arithmetic (`ToxicValue`, `CanonicalToxicToken`, `ToxicTotal`) and is deliberately not wired; the tokens are not in `canonicalKeywords`, so no card can carry them yet.",
	},
	{
	...
	{
		Slug: "extra-turns", Name: "Extra turns", Kind: KindSeam, Status: StatusMissing,
		Summary:     "Spells and abilities that let a player take an extra turn.",
		Missing:     "Extra turns don't exist in the engine yet.",
		Issue:       753,
		Unblocks:    14,
		Waiting:     []string{"Time Warp", "Time Stretch", "Avatar Kuruk"},
		Phrases:     []string{"extra turn"},
		EngineNotes: "primitive: extra turns do not exist. The turn machinery also can't take them as it stands: `Turn.IsNewTurn` compares seats, so a seat taking two turns in a row would not reset its per-turn state, and `Turn.Number` counts rounds, not turns. Planned with extra combats (\"Extra combat and main phases\") as one turn-machinery ADR. Re-checked 2026-09-24: nothing in `internal/game` takes an extra turn.",
	},
	{
	...
	},
	{
		...
		EngineNotes: "**The activated-ability half shipped with #1184**: `CostModifier.Activations` prices ACTIVATED ability costs through the same CR 601.2f pass a cast uses (Boom Scholar, Takenuma), and #1191 widened the pass to mana abilities. Re-checked 2026-09-24, two gaps are left. (1) Training Grounds' \"can't reduce the mana in that cost to less than one mana\" is a floor of ONE mana, and the pass has only the generic floor of zero. (2) \"Dinosaur spells you cast have prowl {2}{R}\" (Hunting Velociraptor) GRANTS an alternative cost to spells cast from hand — a granted `CastPermission` opens a zone and prices it, but nothing grants a keyword cost to a set of spells, and prowl itself has no constructor. No seam issue: the cards are recorded on batch issues #302 and #399.",
	},
	{
	...
	{
		Slug: "resolution-lki", Name: "Remembering a permanent after it leaves", Kind: KindSeam, Status: StatusMissing,
		Summary:     "Abilities that read a creature's power or other details when they resolve, even if the creature has left the battlefield by then.",
		Missing:     "If the creature leaves before the ability resolves, the ability uses what the creature looked like when it triggered, not when it left.",
		Rules:       []string{"608.2h"},
		Issue:       1379,
		Phrases:     []string{"before the ability resolves"},
		EngineNotes: "primitive: no resolution-time last-known information for a permanent that has already left the battlefield. `Game.lastKnownBattlefield` and `LastKnownCountersForEffect` live for one trigger dispatch only, and `LookupCardForEffect` on a moved card returns its new-zone state. `stack_lki.go` (`lastKnownStack`) is the model for spells. Cream of the Crop ships with a caveat for it. Added from #1379 (2026-09-24).",
	},
	{
		Slug: "becomes-plotted", Name: "\"When this card becomes plotted\"", Kind: KindSeam, Status: StatusMissing,
		Summary:     "Plot works; some plot cards also have an ability that triggers when you plot them.",
		Missing:     "Abilities that trigger when a card becomes plotted don't work yet.",
		Rules:       []string{"702.170"},
		Issue:       1382,
		Waiting:     []string{"Longhorn Sharpshooter", "Aloe Alchemist"},
		EngineNotes: "event: plotting a card (the CR 116.2 special action, `SpecialActionPlot`) emits no event a triggered ability can watch from exile, so \"when this card becomes plotted\" has nothing to trigger on. Added from #1382 (2026-09-24).",
	},
	{
		Slug: "commander-ninjutsu", Name: "Commander ninjutsu", Kind: KindSeam, Status: StatusMissing,
		Summary:     "Ninjutsu works from your hand; commander ninjutsu also works from the command zone.",
		Missing:     "A commander can't use ninjutsu from the command zone yet.",
		Rules:       []string{"702.49c"},
		Issue:       1278,
		Waiting:     []string{"Yuriko, the Tiger's Shadow"},
		EngineNotes: "primitive: the activation surface already walks the command zone (#1221), and `putOntoBattlefieldFromZoneLocked` is generic in its source zone; what is not examined is the commander bookkeeping around a command-zone exit that is not a cast, and `effects.ninjutsuEnter` refuses anything not in the activator's hand. Added from #1278 (2026-09-24).",
	},
	{
