...
var items = []Item{
	...
	{
		Slug: "departed-damage-source-keywords", Name: "Lifelink and deathtouch from a source that has left", Kind: KindSeam, Status: StatusMissing,
		Summary:     "Damage dealt by a creature that has already left the battlefield still counts its last known power.",
		Missing:     "That damage doesn't carry the lifelink or deathtouch the creature had when it left.",
		Issue:       1396,
		ADR:         "0056-infect-wither-toxic.md",
		Waiting:     []string{"Warstorm Surge", "Murderous Redcap"},
		Phrases:     []string{"without any lifelink or deathtouch"},
		EngineNotes: "Narrowed from \"Post-departure LKI / exiled with this record\" once #1379 delivered durable resolution-time LKI (`Game.lastKnownPermanents`, `PermanentForEffect(ObjectRef)`). The one open piece is ADR 0056 Decision 2 step 4: `effectDamageTailLocked` reads a source's keywords off the battlefield only, so damage from a source that has left carries none of its lifelink or deathtouch. The record it needs now exists.",
	},
	{
