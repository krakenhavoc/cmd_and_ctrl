...
- Env vars:
  ...
  - `CMDCTRL_BOT_DECISION_LOG_MODE` — `escalated` (default: full board view only for windows that left Layer A) | `all` | `model` (only the windows a model answered). An unrecognised value fails the boot, like `CMDCTRL_BOT_MAX_THINK`.
